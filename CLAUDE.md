...
- Continuam **fora** do VS: loot/raridade/níveis de personagem, árvore de talentos, percepção graduada, fast travel. **Isso continua valendo para a BUILD do edital**, que sai de `develop_manager` — o Vertical Slice completo e verde. O trabalho de itemização a dado, afixos rolados, física de impacto e Mão Secundária vive em `develop_items` e é **pós-VS por construção**: as duas branches são separadas de propósito, para a ambição de uma não pôr a entrega da outra em risco.
- **Progressão por nível: reafirmado FORA do VS em 2026-08-18.** O motor existe e funciona (`Progressao` + `ProgressionBridge`, ligado desde a Fase 3), mas **ninguém concede Exposição no mundo e não há um único `EcoDef` autorado** — e pôr isso de pé atrasaria o edital. Decisão do Vini: não incluir níveis nem talentos no Vertical Slice; quando o VS estiver completo, avaliar expô-los pelo **Carcosa Debugger** primeiro. **Consequência que precisa ficar clara:** com o nível travado em 1, o loot **só entrega tier 1** — isso é esperado no VS, não bug. Não "consertar" autorando tiers altos nem criando fontes de Exposição às pressas. **Atualização de 2026-08-27:** segue fora do VS, mas deixou de ser opcional em `develop_items` — o modelo de afixos rola por **nível do item**, e sem alguém concedendo Exposição no mundo o gerador entrega sempre o piso do pool. Conceder Exposição ao abater entra na Fase 5 daquele plano. **Isso não muda a build do edital.**
