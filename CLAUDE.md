...
- Gênero: **combate + exploração com horror cósmico** (referência de estrutura: Zelda), decidido pelo Vini em **2026-09-01**. Dificuldade moderada. O horror cósmico permanece como núcleo **tonal e narrativo**; a **furtividade deixa de ser pilar e vira ferramenta opcional**.

  > **Por que isto custou pouco para decidir.** Não foi uma virada: foi a documentação alcançando o código. Medido em 2026-08-31: a única ferramenta de furtividade que existe é a tempestade abafando som, e o Deserto de Hali tinha **um** obstáculo dentro da área jogável — sem cobertura e sem rota alternativa, não havia decisão furtiva a tomar. Enquanto isso o combate já tinha faixa de dano branco, crítico, precisão, escala por nível do item, afixos rolados, curva de raridade, mitigação em dois canais e chefes com FSM. **Todos os relatos de playtest do Vini foram relatos de combate.**

  > **O que isto NÃO revoga:** a terminologia diegética (skill `favela-lore-enforcer`), a Resiliência Mental como segundo canal de dano e segundo vetor de derrota, nem a Tempestade de Memória — que ganhou papel novo como véu que esconde o Templo (ver `systems/navegacao_e_tiles.md` e o Véu da Tempestade).

  > **O que fica caro e ainda não foi feito:** detecção **visual** de inimigo. A percepção hoje é 100% sonora; visão exige cone, linha de visão e oclusão por geometria. É sistema, não ajuste — e não estava no prazo do edital.

- Herança: o **combate aberto já era um pilar sistêmico próprio** antes desta decisão, não um mero fallback "quando o stealth falha": toda unidade tem uma ficha de atributos (Vitalidade/Ataque/Defesa/Conjuração/Resistência Anômala), uma fórmula de mitigação de dano por defesa, e armas com ataque básico + habilidade própria (cooldown independente) — ver `Docs/KnowledgeBundle/systems/ficha_de_atributos.md`. O jogo também vai incorporar outros sistemas de progressão (inventário, barra de ações, árvore de talentos/habilidades, loot com raridade e níveis de personagem) por decisão explícita do Vini — ver os bullets abaixo para o que já foi decidido e o que ainda precisa de confirmação. **Não proponha mecânicas de ARPG novas por conta própria** além das já decididas; qualquer outra (ex.: moeda, crafting) precisa ser confirmada com ele antes.
