...
- Loot, raridade e níveis de personagem: **previstos, mas FORA do Vertical Slice atual** (decisão de 2026-07-29). Vai ter drop de item, raridade e progressão por nível, com escopo **deliberadamente contido**: o Vini quer evitar profundidade de build tipo Path of Exile / Last Epoch — nada de dezenas de afixos por item nem explosão de builds divergentes. Mas isso entra **depois de o Vertical Slice ser entregue** — não propor nem implementar enquanto o VS estiver em aberto. A **forma foi desenhada em 2026-08-10** (ver `systems/loot_e_drop.md`): tabela de drop por arquétipo em todo inimigo e baú, 4 graus diegéticos de impregnação, e a regra central de que o sorteio escolhe *qual* `ItemDef` cai mas **nunca gera atributos** — é ela que impede a explosão de build. Continua **não implementado e fora do VS**: não começar sem pedido explícito. Curva de níveis e a contrapartida do grau alto seguem em aberto.
