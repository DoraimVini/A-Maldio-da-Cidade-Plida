...
gdd_unified_path = r'c:\Users\Vini\Desktop\projeto_amarelo_unity\Docs\GDD_Unificado_Favela_Amarela.md'

content = """# A MALDIÇÃO DA CIDADE PÁLIDA (FAVELA AMARELA)
...
* **Universo Transmídia:** *Favela Amarela* (Curta-Metragem, Animação *"O Peregrino Amarelo"*, HQ e Longa-Metragem)
* **Criador / Lead Designer:** Vinícius (Vini) & Tiago Magalhães Rosa (Tiago Tuchu/Gutux)
...
3. **Mão Física & Sorteio de Armas Rituais (RNG):** Damião começa desarmado (golpe de mão vazia com Dano 0). Encontra no Baú da Tumba uma de três armas rituais únicas com habilidades próprias. *(Nota: O Salto Dimensional foi completamente removido do jogo).*
...
* **Detecção por Faro (Sseth Farejador):** Inimigos com faro detectam Damião por proximidade em área, independente de som ou visão. Podem ser neutralizados pelo *Frasco de Incenso*.
...
Essa fórmula impede que defesas altas anulem 100% do dano, garantindo que todo golpe cause ao menos 15% de dano residual. Damião possui números de dano flutuantes (*DanoFlutuante*) em world space para feedback visual imediato.
...
*(Nota de Design: O Salto Dimensional/Ghost Dash foi integralmente removido do jogo. Não há poderes de teleporte atravessando paredes. Damião depende de esquiva física e das 3 armas rituais).*
...
- **Caminho A (Concordar com Alhazred):** Alhazred libera Yug-Neth e desaparece com o *Necronomicon*. Sem luta. Damião obtém o companheiro, mas perde a capacidade de traduzir Aklo.
...
with open(gdd_unified_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("GDD Unificado v3.0 Devlog Sincronizado gerado com sucesso!")
