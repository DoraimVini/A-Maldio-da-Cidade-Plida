...
- [Auditoria de Hitbox e Hurtbox](auditoria_hitbox_hurtbox.md) — **[NOVO, 2026-09-03]** Varredura de TODA consulta de física, trigger, i-frame e knockback em `Assets/Scripts`, com forma, tamanho, offset, momento, máscara e continuidade. **Achado principal:** a migração para hitbox com janela ativa parou no Byakhee — o Cultista e o Esqueleto ainda acertam por proximidade radial instantânea
- [Auditoria de Rigidbody2D](auditoria_rigidbody2d.md) — **[GERADO POR FERRAMENTA, 2026-09-04]** Tipo de corpo, gravidade, massa, amortecimento, travas, detecção, interpolação e sono de todos os 35 corpos, mais os pareamentos corpo↔colisor suspeitos. Rode `Tools/FavelaAmarela/Auditar Física 2D` para atualizar — edições à mão são perdidas
- [Auditoria de Colisores 2D](auditoria_colisores.md) — **[GERADO POR FERRAMENTA, 2026-09-04]** Forma, tamanho em mundo, offset, trigger, composição e material dos 141 colisores, comparados **por papel** (hurtbox / pegada de ator / cenário / gatilho) e não contra o sprite cru. **Achado principal:** a hurtbox da instância do Abdul na `Tumba_De_Alhazred` está a 2,61 unidades do centro desenhado, e as 10 do Cultista saem 43% mais estreitas do que `Hurtbox.GarantirPara` produziria
- [Auditoria de Gatilhos e Callbacks](auditoria_gatilhos.md) — **[GERADO POR FERRAMENTA, 2026-09-04]** Os 84 triggers do projeto, o propósito de cada um, e se o callback casa com o colisor. **Achados:** nenhum script declara `OnCollision*2D` (o modelo do jogo é consulta, não sobreposição), e o Byakhee tem dois gatilhos órfãos na instância de cena que comem slots do buffer de interação
