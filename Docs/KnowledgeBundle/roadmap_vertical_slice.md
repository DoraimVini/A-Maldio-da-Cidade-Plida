...
|---|---|---|
| ~~**Áudio: ausente**~~ **Áudio: existe e está ligado** ⚠️ **corrigido em 2026-08-20** | A linha anterior dizia "zero `AudioSource`/`AudioClip` no gameplay, zero arquivos de som" e concluía que *"hoje se joga um stealth sonoro sem ouvir nada"*. **Estava errada.** Existe uma camada de áudio **sintetizado** (`SinteseDeSom` gera as formas de onda em runtime, por isso zero arquivos `.wav` é o esperado, não um sintoma): `MixerDeAudio` está em **5 cenas**, e `AudioDeStealth` e `AudioDeResiliencia` também — as cinco de gameplay. Conferido por GUID no YAML. | **O pilar sonoro toca.** Sobram dois buracos reais, bem menores que o alegado: **(a)** `AudioDeCombate` está só no `Cultista.prefab` — Byakhee, Rei em Amarelo, Abdul, Espectro e Esqueleto **lutam em silêncio**; **(b)** `SomDoJogo` não está em lugar nenhum. ⚠️ **Remedido em 2026-09-04, e as duas metades estavam erradas.** **(b)** é falso achado: `SomDoJogo` é um **enum** (`Assets/Scripts/Audio/SomDoJogo.cs`) — procurar por ele numa cena não podia dar outro resultado. **(a)** é verdadeiro mas a causa não é a alegada: o Byakhee ganhou `AudioDeCombate` desde então, e os outros quatro **não podem recebê-lo**, porque ele traz `[RequireComponent(typeof(EnemyBase))]` e só dois prefabs do projeto têm `EnemyBase` (Cultista e Byakhee) — Abdul, Rei, Esqueleto e Espectro implementam dano por conta própria. O buraco real é mais fundo e mais barato de fechar: o som de **impacto** saiu de `AudioDeCombate` e foi para `Hurtbox.Receber`, o único ponto por onde todo golpe que acerta passa nos dois sentidos. Ver [log.md](log.md). |
...
| 2 | **Sistema de Consumíveis** | ✅ **Fechado** (2026-08-12) | `InventoryManager` + `ItemDatabase` + `ItemDef`, `EquipmentInventory` com 7 slots, `BarraDeItens` na HUD (teclas 1–8) e `ConsumirItem` funcionando. **3 consumíveis autorados** (Água da Cacimba → corpo, Erva de Ancoragem → mente, Raiz de Yhtill → os dois) e **9 instâncias espalhadas no Deserto** via `Tools/FavelaAmarela/Montar consumíveis do Deserto`. Modelo: **finitos, não farmáveis**, com o anti-*soft-lock* no `RefugioDeLuz` (que agora cura 40% da Vitalidade além da RM cheia) em vez de moeda ou recarga. ⚠️ A anotação anterior dizia `grep "Tipo: 3" devolve zero` — era **factualmente errada**, os 3 já existiam. Ver [inventario_e_consumiveis.md](systems/inventario_e_consumiveis.md). ⚠️ **2026-09-10:** o `InventoryManager.prefab` tinha **seis** slots serializados contra sete no C# — o escudo (Mão Secundária) **nunca pôde ser equipado por ninguém** desde que existia, e a suíte estava verde porque nenhum teste equipava um escudo. Corrigido (`OEscudoEquipaTests`). O Broquel de Couro é entregue **garantido** pelo Baú de Yhtill (`Drop_BauDeYhtill`). Plano A da UI do inventário aplicado no mesmo dia: rótulos do Corpo visíveis e diegéticos ("Mão Secundária — Broquel…"), títulos que cabem, janela opaca, ficha sem nome de enum (`NomesDeAtributoTests`). Fica para depois: grade por `LayoutGroup` e painel do item selecionado (plano B). |
...
| 9 | **Boss Byakhee** | ✅ **Core, Runtime e prefab prontos** (2026-08-12) — `ByakheeFSM` (3 fases, 10 testes), `ByakheeAI`, `Ficha_Byakhee` e `Drop_Byakhee` com o Anel do Sinal Amarelo garantido — **a tabela existia e não estava ligada a nada** até 2026-08-19: o `DropAoAbater` só vivia no `Cultista.prefab`, então o Byakhee morria sem largar o Anel e o rito do Rei era impossível de fechar em jogo. Corrigido por `LigarDropDoByakhee` + guarda `ReliquiasDoRitoTests`. `Byakhee.prefab` com spritesheet animado real (26 frames, 6 animações nomeadas) e o `AnimadorDoByakhee` ligado e populado. **Arena em cena desde 2026-08-20** (`Portoes_Das_Ruinas.unity`, alcançável pelo marco do Deserto; luta por gatilho, Portões destrancados no abate e abertos por interação). **Falta:** só a cena de abertura (o grito antes da forma). O `AnimadorDoByakhee` **já está no prefab e populado** com os 27 sprites — a menção anterior a "ligar o `Animator`" estava obsoleta, e pior, apontava para o caminho errado: há um guarda que **proíbe** `Animator` neste prefab. (O papel de Yug-Neth como "chave dimensional" foi **descartado** em 2026-08-20 — no lugar, o fim da luta libera um **Poste de Luz**, que já reanima o companheiro, ancora a RM, cura e grava a partida.) Ver [systems/boss_byakhee.md](systems/boss_byakhee.md). ⚠️ **Correção grave (2026-08-21):** apesar de "pronto" desde 12/08, o prefab **não tinha colisor nenhum** — só `Rigidbody2D`, sem `Collider2D`. Como `MaoFisicaBridge.ResolverGolpe` resolve o golpe por `Physics2D.OverlapCircle`, o chefe era **estruturalmente impossível de acertar**: o item marcava ✅ enquanto o VS terminava num boss invencível. Corrigido (cápsula trigger) e guardado por `ColisoresDoElencoTests.OByakhee_TemColisorEPodeSerAcertado`. A mesma auditoria de física achou e corrigiu: 4 `Rigidbody2D` (Byakhee + os 2 Cortesãos Pálidos do Castelo) sem `FreezeRotation` — giravam ao colidir; `Physics2D.simulationMode` global divergindo da convenção `FixedUpdate` de todo o código de movimento; e o layer `Player` colidindo fisicamente com `Enemy` (empurrão sem função de jogo, já que dano é resolvido por distância). ⚠️ **2026-09-10, quatro correções medidas:** (1) o `AnimadorDoByakhee` **nunca ouviu a FSM** (inscrição no `OnEnable` antes do Awake alheio) — a luta inteira tocava a espreita; (2) a folha alternava o lado a cada quadro e depois o torso saltava até 22 px — espelhada e alinhada, com guardas na folha (`AFolhaDoByakheeTests`); (3) o abate **não persistia** — sair e voltar era farm infinito; agora `Quest.Portoes.ByakheeAbatido` (`OByakheeNaoEhFarmTests`); (4) o mergulho **vibrava em cima do Damião** (46 inversões em 10 s) e o rasante seguia 12 un além dele — `Mergulhar()` para no alvo e `EncerrarRasante()` encerra ao passar 3 un do jogador ou bater na muralha. **Aberto, decisão do Vini:** a cada fase ela mira o Damião e o atravessa, invertendo a direção a cada 1–2 s; opções na mesa: arco, ou `Circundando` desde a fase 1. Movimento medido com o `DetectorDeOscilacao` (novo, em `Diagnostico/`). |
...
| # | Item | Estado |
|---|---|---|
| 11 | **Blockout do Castelo** | ✅ **Greybox jogável** (2026-08-19) — `Castelo_Carcosa.unity` existe, está no Build Settings e é alcançável pelos **Portões das Ruínas** — o atalho direto do Santuário, que existia só porque o Castelo era cena solta, foi **removido em 2026-08-20**: ele pulava o Byakhee, a única fonte do Anel do Sinal Amarelo, e levava ao Rei sem o que é preciso para vencê-lo. Quatro zonas do caminho crítico montadas por `MontarCasteloCarcosa`: **Z1 Portões** (chegada + Refúgio), **Z2 Salão do Banquete** (6 nobres fossilizados como cobertura + 2 `CortesaoPalido` patrulhando), **Z3 Biblioteca** (3 Espelhos de Aldebaran com `PressaoPsiquicaZone` + 2 `EcoDeCarcosa`), **Z5 Trono** (Rei + 3 pontos focais). **Z4 Observatório fica de fora seguindo o design** — é dungeon opcional, aberta só com o Set Lendário 4/4. **O achado que destravou:** `PressaoPsiquicaZone`, `CortesaoPalido`, `EcoDeCarcosa`, `PontoFocalDeReliquia` e `DetectorDeCostas` já estavam **todos escritos e em cena nenhuma** — o Castelo era ligação, não sistema novo. **Falta:** vestir com arte (ver nota abaixo) e playtest. Guarda: `CasteloDeCarcosaTests` (4 testes). ⚠️ **Parcial em 2026-09-03: os 3 pontos focais da Z5 foram vestidos.** Estavam com `spriteInativo` **e** `spriteAtivo` vazios nos três — ativar uma relíquia não mudava um pixel, e o Vini relatou isso como "os altares não estão funcionando". Ganharam base de pedra (`Ruin_3`, CraftPix Undead) e um feixe amarelo de 10 quadros (`AnimadorDeAltarDeReliquia`). **Continuam sem arte:** os 6 Nobres Fossilizados e os 2 Cortesãos da Z2, os 3 Espelhos de Aldebaran e os 2 Ecos da Z3, e o Refúgio da Z1. Guarda nova: `AltaresRespondemNaTelaTests` (2 testes). ⚠️ **Remedido em 2026-09-04 resolvendo cada `m_Sprite` por GUID, e a lista acima estava errada em quatro dos seis itens.** Os **2 Cortesãos** têm arte autorada (`CortesaoPalido_Front`); o **Refúgio** ganhou o Poste de Osso, e `RefugioDeLuz` escreve o sprite no `Awake` — no YAML ele parece vazio e não está; os **2 Ecos** eram pior que greybox e viraram trabalho próprio (abaixo). O que de fato continua desenhando o **sprite embutido da Unity** (`fileID: 10905`), sem nenhum componente que troque isso em runtime, são os **6 Nobres Fossilizados** e os **3 Espelhos de Aldebaran** — 9 quadrados brancos. Candidatos já no repo, a PPU 32 e Point: `CraftPix/Undead/Objetos/Crystal_1.png` (2 × 2 un) para os Espelhos; para os Nobres o pacote não tem estátua, então é desenhar. 🔴 **E os 2 Ecos de Carcosa não eram falta de arte, eram defeito:** eles tinham **zero filhos e nenhum `SpriteRenderer`**, e `EcoDeCarcosa.AtivarEco()` mostra o vulto ligando os **filhos** do objeto. O Eco se manifestava, drenava 3 de Resiliência Mental por segundo e nada aparecia na tela — num jogo em que Resiliência zerada é derrota. Corrigido em 2026-09-04 com arte própria (`Art/Enemies/EcoDeCarcosa/`), `GarantirVisual()` no padrão de `Hurtbox.GarantirPara`, a ferramenta `Cena: vestir os Ecos de Carcosa` e o guarda `CasteloDeCarcosaTests.CadaEcoDeCarcosa_TemQuadrosParaMostrar`. `OsSistemasDoCastelo_EstaoInstanciados` contava os dois Ecos e passava verde: contar instância não é perguntar se ela funciona. ✅ **Fechado em 2026-09-09:** os 6 Nobres e os 3 Espelhos ganharam arte própria (`Art/Environment/CasteloCarcosa/`, desenhada por script como o Poste e o Eco) pela ferramenta `Cena: vestir o Salão e a Biblioteca do Castelo`, que grava também o `sortingOrder` — ganhar arte cria o problema que o quadrado branco escondia, o de o jogador precisar passar atrás deles. ⚠️ **E a varredura achou mais 6 nus fora do Castelo**, que o "9 quadrados brancos" não contava: os **3 Fragmentos de Yhtill** (itens de quest que o jogador tem de achar), a **Passagem para o Castelo**, o **Piso do Santuário** e o **Visual do Escudo do Abdul**. Congelados em `ArteNasCenasTests.ConhecidosSemArte`, com o custo de cada um escrito — a lista existe para encolher. |
...
| 14 | **Dungeon 2 (Templo da Serpente)** | ❌ **Não começada de fato — verificado em 2026-08-21.** A linha desde 2026-08-19 dizia "começada... não verifiquei ainda se produzem cena jogável" e ficou dois dias sem ninguém checar. Verificado agora: **não existe `Assets/Scenes/*Templo*.unity`** e **nada está no Build Settings**. Os três arquivos (`GeradorCenaTemploSerpente.cs`, `TemploSerpenteSceneBuilder.cs`, `TemploSerpenteSetup.cs`) são ferramentas de montagem nunca executadas — código-scaffolding, não conteúdo jogável. Fora do caminho crítico do VS (item de Prioridade 5, é o destino pós-VS, não bloqueia a build). Design completo em `lore/templo_da_serpente.md`.<br><br>**Pendência de balanceamento herdada (2026-09-09):** o `Ataque` do Cultista caiu de 20 para 16 num ajuste da abertura, e o **Sseth Farejador ficou em 20**. A documentação de fichas dizia que ele batia "igual ao Cultista **de propósito** — ele não é mais forte, ele caça por faro". Essa paridade quebrou. O Vini decidiu **não mexer agora** e trazer a questão para a construção do Templo, que é onde o Sseth entra em jogo: ou ele cai para 16 e a paridade volta, ou os 20 passam a ser uma escolha nova e a razão precisa ser reescrita. O que não pode é ficar como está — número órfão de motivo. |
