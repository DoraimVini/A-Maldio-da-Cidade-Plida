...
- Câmera (`IsometricCameraController`) fica sempre com rotação `Quaternion.identity` — nunca tilte fisicamente a câmera (a "sensação" isométrica vem do Y-sorting e de `PlayerMovement.ConvertToIsometric`, não de rotação de câmera). Ao mexer em física, câmera, prefab de sala ou Rigidbody de inimigo/player, confira a skill `favela-isometric-standards`.
