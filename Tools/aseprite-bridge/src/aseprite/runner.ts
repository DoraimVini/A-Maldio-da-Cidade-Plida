...
function resolveExePath(): string {
  ...
  if (!exePath) {
    throw new AsepriteBridgeError(
      ...
      "Variável de ambiente ASEPRITE_EXE_PATH não configurada. Defina-a na entrada 'aseprite-bridge' do .mcp.json apontando para o executável do Aseprite (ex.: C:\\aseprite\\build\\bin\\aseprite.exe)."
    );
  }
  return exePath;
}
 ...
 * `app.params["chave"]`.
 */
export async function runAsepriteScript(
  logger: Logger,
  filePath: string,
  scriptName: string,
  ...
  const args: string[] = ['-b', resolvedFile];
  ...
  try {
    ...
    throw new AsepriteBridgeError(
      ErrorType.ASEPRITE_EXECUTION,
      `Falha ao rodar script '${scriptName}' contra '${filePath}': ${message}`
    );
  }
}
...
export async function exportFramePng(
  ...
  filePath: string,
  outputPath: string,
  options: { frame?: number; trim?: boolean } = {}
): Promise<void> {
  const exePath = resolveExePath();
  const resolvedFile = path.resolve(filePath);
  ...
  try {
    ...
    throw new AsepriteBridgeError(
      ErrorType.ASEPRITE_EXECUTION,
      `Falha ao exportar preview PNG de '${filePath}': ${message}`
    );
  }
}
