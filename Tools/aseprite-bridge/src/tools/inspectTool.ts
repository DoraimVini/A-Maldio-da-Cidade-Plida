...
interface InspectResult {
  ...
  frames: number;
  layers: string[];
  tags: { name: string; fromFrame: number; toFrame: number }[];
  slices: { name: string; x: number; y: number; width: number; height: number }[];
}

function parseInspectOutput(stdout: string): InspectResult {
  const result: InspectResult = { width: 0, height: 0, frames: 0, layers: [], tags: [], slices: [] };
...
export function registerInspectTool(server: McpServer, logger: Logger) {
