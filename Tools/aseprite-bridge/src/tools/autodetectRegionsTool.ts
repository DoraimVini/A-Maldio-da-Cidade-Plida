...
const toolDescription =
  'Detecta regiões de conteúdo real numa imagem .aseprite achatada, sem depender de grid fixo. ' +
...
function parseRegionsOutput(stdout: string): { regions: Region[]; total: number } {
...
export function registerAutodetectRegionsTool(server: McpServer, logger: Logger) {
  ...
  server.tool(toolName, toolDescription, paramsSchema.shape, async (params: z.infer<typeof paramsSchema>) => {
    try {
      ...
      if (params.frame !== undefined) scriptParams.frame = String(params.frame);
      if (params.minAlpha !== undefined) scriptParams.minAlpha = String(params.minAlpha);
      if (params.minSize !== undefined) scriptParams.minSize = String(params.minSize);
