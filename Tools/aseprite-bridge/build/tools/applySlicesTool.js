...
const toolDescription = 'Adiciona Slices (metadata, não mexe nos pixels) num .aseprite nas coordenadas informadas. ' +
...
function encodeSlices(slices) {
    return slices
        .map((s) => `${s.name},${s.x},${s.y},${s.width},${s.height}`)
        .join(';');
}
export function registerApplySlicesTool(server, logger) {
        ...
        try {
            ...
            for (const slice of params.slices) {
                if (slice.name.includes(',') || slice.name.includes(';')) {
                    throw new AsepriteBridgeError(ErrorType.VALIDATION, `Nome de slice inválido: '${slice.name}'. Não pode conter ',' ou ';' (usados como delimitador ao chamar o Aseprite).`);
                }
            }
