...
const toolDescription = 'Converte cada Slice existente (criada antes via aseprite_apply_slices) num Frame real, ' +
...
function encodeTagGroups(groups) {
    if (!groups || groups.length === 0)
        return '';
    for (const g of groups) {
        if (g.name.includes(',') || g.name.includes(';')) {
            throw new AsepriteBridgeError(ErrorType.VALIDATION, `Nome de tag inválido: '${g.name}'. Não pode conter ',' ou ';'.`);
        }
    }
    return groups.map((g) => `${g.name},${g.fromIndex},${g.toIndex}`).join(';');
}
export function registerPromoteToFramesTool(server, logger) {
